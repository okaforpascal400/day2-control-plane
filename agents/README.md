...
| **Human-in-the-loop** | The refusals in the table above, plus the platform backstop: GitHub does not trigger workflows for events created with `GITHUB_TOKEN`, so an agent-opened PR cannot even start CI on itself. A human re-runs it, reviews, and merges. | `core/day2_agents/guardrails.py`; tested in `core/tests/test_guardrails.py` |
